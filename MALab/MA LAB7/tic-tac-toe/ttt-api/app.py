...
def home():
    ...
    if id:
        ...
        rw.add_form(BASE_URL + "register", method_name="POST",
                    contentType="application/json", op="writeproperty")
...
def register():
    ...
    rw.add_form(BASE_URL + "registerGame", method_name="POST",
                contentType="application/json", op="writeproperty")
    rw.add_form_property(BASE_URL + "registerGame",
                         GAME_ONTOLOGY_PREFIX+":XPlayerRole", False, True)
    rw.add_form_property(BASE_URL + "registerGame",
                         GAME_ONTOLOGY_PREFIX+":OPlayerRole", False, True)
    return format_response(rw)
...
def play():
    rw = ResponseWriter(BASE_URL + "registerGame",
                        GAME_ONTOLOGY_PREFIX+":Game")
    ...
    if not (primary_agent in FREE_PLAYERS):
        ...
        game_rdf_store = RDFStore(g, GAME_ONTOLOGY_TAG, BASE_URL, URIRef(
            primary_agent), URIRef(second_agent), id)
        ...
        rw.add_field(GAME_ONTOLOGY_PREFIX+":XPlayerRole",
                     URIRef(primary_agent))
...
def board(id):
    ...
    if id in GAMES:
        game_rdf_store = GAMES[id]
        # This adds links to free squares
        add_links_for_active_game(game_rdf_store, rw, id, False)

        board = game_rdf_store.get_board_occupied()
        for b in board:
            # Just passing key in
            rw.add_nested_field(GAME_ONTOLOGY_PREFIX +
                                ":hasSquare", b, board[b])
...
def add_links_for_active_game(game_rdf_store, rw, id, includeBoardLink=True):
    ...
    if (game_rdf_store.is_game_over()):
        ...
        for sq in free_squares:
            rw.add_form(str(sq), method_name="PUT",
                        contentType="application/json", op="writeproperty")
